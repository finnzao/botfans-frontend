...
import { extractErrorCause, internalError } from '@/core/lib/utils';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!tenantId || !phone) return NextResponse.json({ success: false, error: 'Campos obrigatórios: tenantId, phone' }, { status: 400 });
    ...
    if (existing.rows.length > 0 && existing.rows[0].has_credentials) {
      ...
      await db.query(`UPDATE telegram_sessions SET phone = $1, status = 'api_captured', error_message = NULL, updated_at = NOW() WHERE id = $2`, [phone, row.id]);
      const flowId = randomUUID();
      await setFlowState(flowId, { tenantId, sessionId: row.id, phone, apiId: row.api_id, apiHash: row.api_hash_encrypted, step: 'api_captured' });
      await publishToWorker(CHANNELS.TELEGRAM_START_SESSION, { flowId, sessionId: row.id, tenantId, phone, apiId: row.api_id, apiHash: row.api_hash_encrypted });
    ...
    const result = await fetchWithRetry(reqId, 'https://my.telegram.org/auth/send_password', {
      method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ phone }),
    });
    ...
    try { portalData = JSON.parse(portalRaw); } catch {
      return NextResponse.json({ success: false, error: 'Resposta inesperada do Telegram.' }, { status: 502 });
    }

    if (!portalData.random_hash) {
      const errorMsg = portalData.error === 'FLOOD_WAIT' ? 'Muitas tentativas. Aguarde alguns minutos.'
        : portalData.error === 'PHONE_NUMBER_INVALID' ? 'Número inválido. Verifique o formato.'
    ...
    if (existing.rows.length > 0) {
      ...
      await db.query(`UPDATE telegram_sessions SET phone = $1, status = 'awaiting_portal_code', error_message = NULL, updated_at = NOW() WHERE id = $2`, [phone, sessionId]);
    } else {
      const r = await db.query(`INSERT INTO telegram_sessions (tenant_id, phone, status) VALUES ($1, $2, 'awaiting_portal_code') RETURNING id`, [tenantId, phone]);
      sessionId = r.rows[0].id;
    }

    const flowId = randomUUID();
    await setFlowState(flowId, { tenantId, sessionId, phone, randomHash: portalData.random_hash as string, step: 'awaiting_portal_code' });
