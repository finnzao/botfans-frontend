...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (contactId) { conditions.push(`o.contact_id = $${idx}`); params.push(contactId); idx++; }
    const where = conditions.join(' AND ');
    const result = await db.query(
      `SELECT o.*, s.name as service_name, s.category as service_category,
        c.first_name, c.last_name, c.telegram_username
       FROM orders o JOIN services s ON s.id = o.service_id JOIN contacts c ON c.id = o.contact_id
       WHERE ${where} ORDER BY o.created_at DESC`, params
    );
    return NextResponse.json({ success: true, data: { orders: result.rows, total: result.rowCount } });
  } catch (error) {
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (!id || !tenantId || !status) return NextResponse.json({ success: false, error: 'id, tenantId e status obrigatórios' }, { status: 400 });
    const current = await db.query('SELECT status FROM orders WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
    if (current.rowCount === 0) return NextResponse.json({ success: false, error: 'Pedido não encontrado' }, { status: 404 });
    const currentStatus = current.rows[0].status;
    const allowed = VALID_TRANSITIONS[currentStatus];
    if (allowed && !allowed.includes(status)) return NextResponse.json({ success: false, error: `Transição inválida: ${currentStatus} → ${status}` }, { status: 400 });
    ...
    await db.query(`UPDATE orders SET status = $1, notes = COALESCE($2, notes), updated_at = NOW()${extra} WHERE id = $3 AND tenant_id = $4`, [status, notes || null, id, tenantId]);
