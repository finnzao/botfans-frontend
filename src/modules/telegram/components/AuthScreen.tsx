...
export function AuthScreen() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      if (mode === 'register') await register(email, password, ownerName);
  ...
  return (
    <div style={s.page}>
      <div style={s.card}>
        ...
        <p style={s.subtitle}>
          {mode === 'login'
            ? 'Entre para gerenciar sua assistente IA'
        ...
        <form onSubmit={handleSubmit} style={s.form}>
          {mode === 'register' && (
            <div style={s.field}>
              ...
              <input style={s.input} placeholder="Ex: Maria Silva" value={ownerName} onChange={e => setOwnerName(e.target.value)} required />
            </div>
          )}
          <div style={s.field}>
          ...
          <div style={s.field}>
            ...
            <input style={s.input} type="password" placeholder="Mínimo 6 caracteres" value={password} onChange={e => setPassword(e.target.value)} required minLength={6} />
          </div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  input: {
    padding: '11px 14px', fontSize: 14, border: '1px solid var(--border)',
    borderRadius: 'var(--radius-sm)', outline: 'none', background: 'var(--bg-card)',
  ...
  button: {
    padding: '12px', fontSize: 14, fontWeight: 600, background: 'var(--accent)',
    color: '#fff', border: 'none', borderRadius: 'var(--radius-sm)', cursor: 'pointer', marginTop: 4,
  },
