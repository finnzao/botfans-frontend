      ...
      <div style={{ display: 'flex', height: 20, borderRadius: 6, overflow: 'hidden', background: '#f0f2f5' }}>
        {entries.map(([key, val]) => {
          ...
          return (
            <div
              key={key}
              title={`${BUCKET_LABELS[key]}: ${val} (${pct.toFixed(0)}%)`}
              style={{
                width: `${pct}%`,
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
        {entries.map(([key, val]) => (
          <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            <div style={{ width: 8, height: 8, borderRadius: 2, background: BUCKET_COLORS[key] }} />
            <span style={{ fontSize: 11, color: '#6b7280' }}>
              {BUCKET_LABELS[key]}: <strong style={{ color: '#333' }}>{val}</strong>
            </span>
          </div>
        ))}
      </div>
