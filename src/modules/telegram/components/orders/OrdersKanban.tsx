...
interface KanbanColumn {
  ...
  color: string;
  bg: string;
  actions: { label: string; targetStatus: OrderStatus; color: string }[];
}

const COLUMNS: KanbanColumn[] = [
  {
    key: 'new', label: 'Novos', statuses: ['pending_approval'],
    color: '#B87A00', bg: '#FAEEDA',
    actions: [
      { label: 'Aceitar', targetStatus: 'approved', color: '#0F6E56' },
      { label: 'Recusar', targetStatus: 'rejected', color: '#A32D2D' },
    ],
  },
  {
    ...
    actions: [
      { label: 'Marcar Pago', targetStatus: 'paid', color: '#0F6E56' },
      { label: 'Cancelar', targetStatus: 'cancelled', color: '#A32D2D' },
    ],
  },
  {
    key: 'production', label: 'Em Produção', statuses: ['paid', 'in_production'],
    color: '#534AB7', bg: '#EEEDFE',
    actions: [
      { label: 'Marcar Entregue', targetStatus: 'delivered', color: '#0F6E56' },
    ],
  },
  {
...
function contactName(order: Order): string {
...
export function OrdersKanban({ tenantId }: Props) {
  ...
  async function handleAction(orderId: string, targetStatus: OrderStatus) {
    ...
    try {
      const res = await updateOrderStatus(orderId, tenantId, targetStatus);
      if (res.success) load();
    } catch {
  ...
  function getColumnOrders(col: KanbanColumn): Order[] {
    return orders.filter(o => col.statuses.includes(o.status));
  }

  if (loading) {
    return <div style={s.loading}>Carregando pedidos...</div>;
  }
  ...
  return (
    <div>
      <div style={s.header}>
        <div>
          ...
          <p style={s.subtitle}>
            ...
            {totalRevenue > 0 && ` • ${formatPrice(totalRevenue)} recebido`}
          </p>
        </div>
      ...
      {orders.length === 0 && (
        <div style={s.empty}>
          <p style={s.emptyTitle}>Nenhum pedido ainda</p>
          <p style={s.emptyDesc}>Quando um cliente solicitar um serviço pelo chat, o pedido aparecerá aqui.</p>
        </div>
      )}
          ...
          return (
            <div key={col.key} style={s.column}>
              <div style={{ ...s.colHeader, background: col.bg }}>
                ...
                <span style={{ ...s.colCount, color: col.color }}>{colOrders.length}</span>
              </div>

              <div style={s.colBody}>
                {colOrders.length === 0 && (
                  <p style={s.colEmpty}>Nenhum pedido</p>
                )}

                {colOrders.map(order => (
                  <div key={order.id} style={s.card}>
                    <div style={s.cardTop}>
                      <span style={s.cardContact}>{contactName(order)}</span>
                      <span style={s.cardTime}>{timeAgo(order.created_at)}</span>
                    </div>
                    ...
                    {order.custom_details && (
                      <div>
                        ...
                        {expandedOrder === order.id && (
                          <p style={s.cardDetails}>{order.custom_details}</p>
                        )}
                      </div>
                    )}
                      ...
                      <div style={s.cardActions}>
                        {col.actions.map(action => (
                          <button
                            ...
                            onClick={() => handleAction(order.id, action.targetStatus)}
...
const s: Record<string, React.CSSProperties> = {
  ...
  colLabel: { fontSize: 12, fontWeight: 600 },
  colCount: { fontSize: 11, fontWeight: 700, opacity: 0.7 },
  ...
  cardContact: { fontSize: 12, fontWeight: 600, color: '#222' },
  cardTime: { fontSize: 10, color: '#aaa' },
  cardService: { fontSize: 11, color: '#555' },
  cardPrice: { fontSize: 13, fontWeight: 700, color: '#0F6E56' },
