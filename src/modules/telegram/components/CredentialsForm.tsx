    ...
    <div>
      ...
      <form onSubmit={handleSubmit} style={styles.form}>
        <div style={styles.field}>
          ...
          <input type="tel" placeholder="+5511999999999" value={phone} onChange={e => setPhone(e.target.value)} required style={styles.input} />
          <span style={styles.hint}>Formato internacional com código do país</span>
        </div>
        <div style={styles.field}>
          <label style={styles.label}>API ID</label>
          <input type="text" placeholder="12345678" value={apiId} onChange={e => setApiId(e.target.value)} required style={styles.input} />
        </div>
        <div style={styles.field}>
...
const styles: Record<string, React.CSSProperties> = {
  infoBox: { background: '#f0f7ff', border: '1px solid #c5ddf5', borderRadius: 10, padding: '14px 18px', marginBottom: 24 },
  infoTitle: { fontSize: 13, fontWeight: 600, color: '#185FA5', margin: '0 0 8px' },
  infoList: { fontSize: 13, color: '#333', margin: 0, paddingLeft: 18, lineHeight: 1.7 },
  form: { display: 'flex', flexDirection: 'column', gap: 18 },
  field: { display: 'flex', flexDirection: 'column', gap: 4 },
  label: { fontSize: 13, fontWeight: 500, color: '#333' },
  input: { padding: '10px 14px', fontSize: 14, border: '1px solid #ddd', borderRadius: 8, outline: 'none', fontFamily: 'monospace' },
  hint: { fontSize: 11, color: '#999' },
  button: { padding: '12px 24px', fontSize: 14, fontWeight: 600, background: '#185FA5', color: '#fff', border: 'none', borderRadius: 8, marginTop: 8 },
  error: { fontSize: 13, color: '#A32D2D', background: '#FCEBEB', padding: '8px 12px', borderRadius: 6, margin: 0 },
};
