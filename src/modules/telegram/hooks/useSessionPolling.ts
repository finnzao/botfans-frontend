...
export function useSessionPolling(tenantId: string, interval = 3000) {
  ...
  const checkStatus = useCallback(async () => {
    try { const res = await getSessionStatus(tenantId); if (res.success && res.data) { setStatus(res.data.status as SessionStatus); if (res.data.id) setSessionId(res.data.id); } } catch {} finally { setLoading(false); }
  }, [tenantId]);
  function startPolling() { stopPolling(); timerRef.current = setInterval(checkStatus, interval); }
  function stopPolling() { if (timerRef.current) { clearInterval(timerRef.current); timerRef.current = null; } }
  useEffect(() => { checkStatus(); return stopPolling; }, [checkStatus]);
