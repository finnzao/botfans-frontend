...
def cmd_status(args):
    ...
    for f in files:
        ...
        checksum = f["checksum"]
        if name in executed:
            status = "✓ executada" if executed[name] == checksum else "⚠ modificada!"
        else:
            status = "⏳ pendente"
...
def cmd_generate(args):
    ...
    sql = args.sql if args.sql else f"-- Escreva o SQL da migration aqui\n\n"
...
def main():
    parser = argparse.ArgumentParser(description="BotFans — Gerenciador de migrations e schema")
